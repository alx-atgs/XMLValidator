...
public class ObjUtil {
	...
	public static String getContentAsString(String filePath, String charset) {
		...
		if (!Check.file(filePath)) {
			System.out.println("Ошибка в ObjUtil.getContentAsString: входящий файл не найден или не является файлом.");
			return null;
		}
		...
		try {
			...
			System.out.println("ObjUtil.getContentAsString: " + e.getMessage());
			// return null;
	 ...
	 * @param charset  - chatset, example "utf-8"
	 * @return File
	 */
	public static File stringToFile(String inputstr, String targetFilePath, String charset) {
		...
		try {
			...
			if (path.toFile().exists()) {
				...
				System.out.println("stringToFile: The file does not exist");
			}
		} catch (IOException e) {
			System.out.println("stringToFile: " + e.getMessage());
			// e.printStackTrace();
			System.exit(2);
		}
		return fname;
	...
	public static String tempPath() {
		try {
			...
			System.out.println("tempPath: " + e.getMessage());
	...
	public static String getFirstLine(String fileFullPath) {
		...
		if (!Check.file(fileFullPath)) {
			System.out.println("ObjUtil.getFirstLine: проверка на существование файла вернула false");
	...
	public static String getNumline(String fileFullPath, int numLine) {
		...
		String line = "";
		if (numLine < 1) {
			System.out.println("ObjUtil.getNumline: не верно задан номер строки. Текущее значение: " + numLine);
		...
		if (!Check.file(fileFullPath)) {
			System.out.println("ObjUtil.getNumline: проверка на существование файла вернула false");
			// System.exit(2) - for Windows: The system cannot find the file specified.
			// ERROR_FILE_NOT_FOUND
			// System.exit(2);
			return null;
		...
		try {
			...
			System.out.println("ObjUtil.getNumline: Ошибка:\n" + e.getMessage());
		}
		return line;
	}
	...
	public static String tail(String filePath, int lines) {
		...
		if (!Check.file(filePath)) {
			System.out.println("ObjUtil.tail: проверка на существование файла вернула false");
			// System.exit(2) - for Windows: The system cannot find the file specified.
			// ERROR_FILE_NOT_FOUND
			// System.exit(2);
			return null;
	...
	public static String delBOM(String inputString) {
		...
		try (InputStream is = string2inputStream(inputString)) {
			...
			System.out.println("delBOM: " + e.getMessage());
			// e.printStackTrace();
		}
		return backString;
	}
	...
	public static String inputStream2String(InputStream is, String charset) {
		...
		try (ByteArrayOutputStream result = new ByteArrayOutputStream()) {
			...
			System.out.println("isToString: " + e.getMessage());
			// e.printStackTrace();
		}
		return backstring;
	}
...
//		try (InputStream steam = new ByteArrayInputStream(Files.readAllBytes(Paths.get(inputString)))) {
		try (InputStream steam = new ByteArrayInputStream(Files.readAllBytes(fileName.toPath()))) {
			...
			System.out.println("stringToIs: " + e.getMessage());
		}
